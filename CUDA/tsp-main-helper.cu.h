...
void runProgram(char* file_name, int restarts, int version){
    ...
    if ( version == 4) {
        ...
        long double while_bytes = (16 * totIter * while_tot) * 1.0e-6f ;
        long double rest = ((6 * cities + 14) * restarts) * 1.0e-6f;
        long double tot = while_bytes + rest;
        ...
        long double gb_new = tot / (elapsed * 1.0e-3);
