...
int main() {
    ...
    int *is_d, *js_d, *is_h, *to_Matrix_d, *to_Matrix_h;
    ...
    cudaMalloc((void**)&to_Matrix_d, (cities+1)*restarts*sizeof(int));
    to_Matrix_h = (int*) malloc((cities+1)*restarts*sizeof(int));
    ...
    free(is_h); free(tourMatrixIn_h), free(to_Matrix_h);
