...
int partitionEvenI32( const uint32_t B     // desired CUDA block size ( <= 1024, multiple of 32)
    ...
    int* h_out = (int*)malloc(mem_size);
    int* h_ref = (int*)malloc(mem_size);
    cudaMalloc((void**)&d_tmp_small, 2 * MAX_BLOCK * sizeof(int));
    cudaMalloc((void**)&d_tmp_large, 2 * N         * sizeof(int));
    cudaMemset(d_out, 0, N*sizeof(int));

    // dry run to exercise d_tmp allocation
    partition2< EvenInt >( B, N, d_out, d_in, d_tmp_large, d_tmp_small );

    // time the GPU computation
    unsigned long int elapsed;
    struct timeval t_start, t_end, t_diff;
    gettimeofday(&t_start, NULL); 
    uint32_t d_len_true = 0;
    for(int i=0; i<RUNS_GPU; i++) {
        d_len_true = partition2< EvenInt >( B, N, d_out, d_in, d_tmp_large, d_tmp_small );
    }
    cudaThreadSynchronize();
    ...
    elapsed = (t_diff.tv_sec*1e6+t_diff.tv_usec) / RUNS_GPU;
    double gigaBytesPerSec = N  * (2*sizeof(int) + sizeof(int)) * 1.0e-3f / elapsed;
    printf("Filter with predicate EVEN on I32, GPU Execution runs in: %lu microsecs, GB/sec: %.2f\n"
          , elapsed, gigaBytesPerSec);
    ...
    { // sequential computation
        ...
        for(int i=0; i<RUNS_CPU; i++) {
            len_true = 0;
            for(uint32_t i=0; i<N; i++) {
                if (EvenInt::predicate(h_in[i]))
                    len_true ++;
            }
            uint32_t ind_t = 0, ind_f = len_true;
            for(uint32_t i=0; i<N; i++) {
                int ind, el = h_in[i];
                if (EvenInt::predicate(el)) {
                    ind = ind_t; ind_t++;
                } else {
                    ind = ind_f; ind_f++;
                }
        ...
        elapsed = (t_diff.tv_sec*1e6+t_diff.tv_usec) / RUNS_CPU;
        double gigaBytesPerSec = N * (sizeof(int) + sizeof(int)) * 1.0e-3f / elapsed;
        printf("Filter with predicate EVEN on I32, CPU Sequential runs in: %lu microsecs, GB/sec: %.2f\n"
              , elapsed, gigaBytesPerSec);
    }

    { // Validation
        cudaMemcpy(h_out, d_out, mem_size, cudaMemcpyDeviceToHost);
        if(d_len_true != len_true) {
            printf("!!!INVALID!!!: Filter with predicate EVEN on I32, different true-length: CPU: %d, GPU: %d\n"
                  , len_true, d_len_true);
            exit(1);
        }
        for(uint32_t i = 0; i<N; i++) {
            if(h_out[i] != h_ref[i]) {
                printf("!!!INVALID!!!: Filter with predicate EVEN on I32 at index %d, dev-val: %d, host-val: %d\n"
                      , i, h_out[i], h_ref[i]);
                exit(1);
            }
        }
        printf("Filter with predicate EVEN on I32 : VALID result!\n\n");
    }

    free(h_out);
    free(h_ref);
    cudaFree(d_tmp_large);
    cudaFree(d_tmp_small);
...
int main (int argc, char * argv[]) {
    ...
    { // filter with "even" predicate an 32-bit integral array
        partitionEvenI32(B, N, h_in, d_in, d_out);
    }
